...
public record LineInfo(byte[] name, byte[] consumer, int offset, int numAttrs, long flags,
                       LineAttribute[] attrs, int[] padding) implements NativeMemoryLayout {
	...
	public static LineInfo create(MemorySegment memorySegment) throws Throwable {
		...
		if (!memorySegment.equals(MemorySegment.NULL)) {
			lineinfoInstance = lineinfoInstance.fromBytes(memorySegment);
		}
	...
	public LineInfo fromBytes(MemorySegment buffer) throws Throwable {
		...
		for(int i = 0; i < 10; i++) {
			...
			attrs[i] = tmp.fromBytes(attrsMemorySegment.asSlice(LineAttribute.LAYOUT.byteSize() * i, LineAttribute.LAYOUT.byteSize()));
		}
		return new LineInfo(
	...
	public void toBytes(MemorySegment buffer) throws Throwable {
		...
		for (int i = 0; i < attrs.length; i++) {
			attrs[i].toBytes(attrsTmp.asSlice(LineAttribute.LAYOUT.byteSize() * i, LineAttribute.LAYOUT.byteSize()));
		}
	...
	public boolean isEmpty() {
		return name.length == 0 && consumer.length == 0 && offset == 0 && numAttrs == 0 && flags == 0 && attrs.length > 0 && padding.length == 0;
	}
}
