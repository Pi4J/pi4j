...
public record LineConfig(long flags, int numAttrs, int[] padding,
		LineConfigAttribute[] attrs) implements NativeMemoryLayout {
	...
	public static LineConfig create(MemorySegment memorySegment) throws Throwable {
		...
		if (!memorySegment.equals(MemorySegment.NULL)) {
			lineconfigInstance = lineconfigInstance.fromBytes(memorySegment);
		}
	...
	public LineConfig fromBytes(MemorySegment buffer) throws Throwable {
		...
		for(int i = 0; i < 10; i++) {
			...
			attrs[i] = tmp.fromBytes(attrsMemorySegment.asSlice(LineConfigAttribute.LAYOUT.byteSize() * i, LineConfigAttribute.LAYOUT.byteSize()));
		}
	...
	public void toBytes(MemorySegment buffer) throws Throwable {
		...
		for (int i = 0; i < attrs.length; i++) {
			attrs[i].toBytes(attrsTmp.asSlice(LineConfigAttribute.LAYOUT.byteSize() * i, LineConfigAttribute.LAYOUT.byteSize()));
		}
	}

	@Override
	public boolean isEmpty() {
		return flags == 0 && numAttrs == 0 && padding.length == 0 && attrs.length > 0;
	}
}
