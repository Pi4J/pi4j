...
public record LineRequest(int[] offsets, byte[] consumer, LineConfig config, int numLines,
                          int eventBufferSize, int[] padding, int fd) implements NativeMemoryLayout {
	...
	public static LineRequest create(MemorySegment memorySegment) throws Throwable {
		...
		if (!memorySegment.equals(MemorySegment.NULL)) {
			linerequestInstance = linerequestInstance.fromBytes(memorySegment);
		}
	...
	public LineRequest fromBytes(MemorySegment buffer) throws Throwable {
		...
		var config = LineConfig.createEmpty().fromBytes(configMemorySegment);
	...
	public boolean isEmpty() {
		return offsets.length == 0 && consumer.length == 0 && config.isEmpty() && numLines == 0 && eventBufferSize == 0 && padding.length == 0 && fd == 0;
	}
}
